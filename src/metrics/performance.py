...
def equity_curve_metrics(equity: pd.Series) -> dict:
    if len(equity) < 2:
        return {"sharpe_daily": float("nan"), "max_drawdown_pct": 0.0,
                "max_dd_duration_days": 0, "annualized_return_pct": 0.0,
                "annualized_vol_pct": 0.0}
    daily_ret = equity.pct_change().dropna()
    sharpe_daily = (daily_ret.mean() / daily_ret.std() * np.sqrt(252)) if daily_ret.std() > 0 else float("nan")
...
def combined_metrics(trades: list[Trade], equity: pd.Series, starting_equity: float = 100_000.0) -> dict:
    ...
    out.update(equity_curve_metrics(equity))
    return out
